...
class MGAN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        ctx_out = self.location(ctx_out, aspect_in_text).to(torch.float32) # batch_size x (ctx)seq_len x 2*hidden_dim
...
#     text_raw_indices = torch.LongTensor(2, 20) # batch_size x seq_len
#     text_raw_indices[:,8:11] = 0
#     aspect_indices = torch.LongTensor(2, 20)
#     aspect_indices[:,2:] = 0
#     text_left_indices= torch.ones(2, 20).long()
#     text_left_indices[:,8:] = 0
#     inputs = [text_raw_indices, aspect_indices, text_left_indices]
#     outs = 